...
$start_dayb1 = date('Y-m-d 00:00:00', strtotime("-1 day")) ; // from yesterday 0:00
$end_dayb1 = date('Y-m-d 23:59:59', strtotime("-1 day"));   // to  23:59

$start_curhour = date('Y-m-d H:00:00') ; // set last hour
$end_curhour = date('Y-m-d H:59:59') ;   // set last hour
$desc_curhour1 = date('Y-m-d H:00') ; // description last hour
$desc_curhour2 = date('H:59') ;       // description last hour
...
//$res = $connection->query("SELECT queuename FROM queues_new");
...
$res = mysqli_query($connection, $query);
//$res = $connection->query("SELECT agent FROM agents_new");
while ($row = mysqli_fetch_row($res)) {
  $agentes[] = $row[0];
}
...
echo "    alt=\"$start_curhour - $end_curhour\" ";
echo "  title=\"$start_curhour - $end_curhour\"";
...
<input type=submit value="Queues">
</form>
</td><td>
<form method="post" action="sync.php">
<input type=hidden name="agent_sync">
<input type=submit value="Agents">
</form>
</td></tr></table>
	</div>
</div>

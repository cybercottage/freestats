...
if(isset($_POST['pagerows'])) {
   ...
   $_SESSION['freestats']['pagerows'] = 1;
}   

if(isset($_POST['callerid'])) {
   $callerid = $_POST['callerid'];
   $_SESSION['freestats']['callerid']=$callerid;
} 

if(isset($_POST['uniqueid'])) {
   $uniqueid = $_POST['uniqueid'];
   $_SESSION['freestats']['uniqueid']=$uniqueid;
} 

if(isset($_POST['event'])) {
   $event = $_POST['event'];
   $_SESSION['freestats']['event']=$event;
} 

if(isset($_POST['ringnoanswer'])) {
   $ringnoanswer = $_POST['ringnoanswer'];
   $_SESSION['freestats']['ringnoanswer']=$ringnoanswer;
} else {
$ringnoanswer = 'FALSE';
}
...
<form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" id="frm1" name="frm1" method="post">
  ...
  <option selected="<?php echo $_SESSION['freestats']['pagerows'];?>"><?php echo $_SESSION['freestats']['pagerows'];?></option>
    ...
    &nbsp;&nbsp;&nbsp;<button type="submit" name="submit"><?php echo $lang["$language"]['filter'] ?> </button>
		...
		<a name='raw'></a>
			<TABLE width='99%' cellpadding=1 cellspacing=1 border=0 class='sortable' id='tableraw'>
				<THEAD>
				<CAPTION><?php echo "Queue log data for the selected period";?></CAPTION>			
                <TR> 
